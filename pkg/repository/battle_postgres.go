...
func (r *BattlePostgres) Create(battle carsBrandRandomGenerator.Battle) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
	...
	row = r.db.QueryRow(createScore1, battle.Player1Id, id, 0)
	if err := row.Scan(&id); err != nil {
		tx.Rollback()
		return 0, err
	}
	createScore2 := fmt.Sprintf("INSERT INTO %s (userId, battleId, playerScore) values ($1, $2, $3) RETURNING id", scoresTable)
	row = r.db.QueryRow(createScore2, battle.Player2Id, id, 0)
	if err := row.Scan(&id); err != nil {
		tx.Rollback()
		return 0, err
	}
...
func (r *BattlePostgres) GetAll() ([]carsBrandRandomGenerator.Battle, error) {
	var battles []carsBrandRandomGenerator.Battle
	query := fmt.Sprintf("SELECT * FROM %s", battlesTable)
	err := r.db.Select(&battles, query)
	return battles, err
}

func (r *BattlePostgres) GetById(id int) (carsBrandRandomGenerator.Battle, error) {
	var battle carsBrandRandomGenerator.Battle
	query := fmt.Sprintf("SELECT * FROM %s WHERE id= $1", battlesTable)
	err := r.db.Select(&battle, query, id)
	return battle, err
}

func (r *BattlePostgres) Update(id int, battle carsBrandRandomGenerator.UpdateBattleInput) error {
